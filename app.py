...
# (Just append this directly to the bottom of your original script)
# ==============================================================================
...
# API 5L X52 / Lower grade parameters
    ...
    # 4. Quantify the exact safety cushion gap
    conservatism_gain = ((P_ansys_projected - base_pressure) / base_pressure) * 100

    # 5. Render comparative layout columns
    col_f1, col_f2, col_f3 = st.columns(3)
    
    with col_f1:
        st.markdown(
            f"<div style='background-color: #f8f9fa; padding: 15px; border-radius: 10px; border-left: 5px solid #007bff;'><b>Pipe Profile Evaluated</b><br><h3>{m_label}</h3></div>", 
            unsafe_allow_html=True
        )
    ...
    st.info(
        f"💡 **Thesis Validation Mapping:** Traditional evaluation equations drop capacity calculations too aggressively "
